...
setlocal indentkeys+==endmodule,=endinstance,=endfunction,=endtask,=endspecify
...
function GetBSVIndent()
  ...
  elseif last_line =~ '^\s*\<\(function\|task\|class\|package\)\>' ||
	...
	if curr_line =~ '^\s*\<\(interface\)\>'
	  echo vverb_str "No indent if interface after interface."
	elseif last_line !~ '\<end\>\s*' . vlog_comment . '*$' ||
      \ last_line =~ '\(//\|/\*\).*\(;\|\<end\>\)\s*' . vlog_comment . '*$'
      let ind = ind + offset
	  if vverb
